...
async function runMigrations() {
  try {
    ...
    const response = await fetch(sqlEndpoint, {
      ...
      body: JSON.stringify({ sql: fullMigrationSQL })
    })

    if (response.ok) {
      console.log('✅ All migrations completed successfully!')
      console.log('💡 Database is now ready with all tables and RLS policies')
    } else {
      const errorText = await response.text()
      console.warn('⚠️  Migration completed with warnings:', errorText)
      console.log('💡 Note: Some migrations may have failed due to missing permissions')
    }
  } catch (error) {
    ...
    console.log('💡 Copy the SQL from this script and paste it there')
    process.exit(1)
  }
}
