...
const migrations: Array<{ name: string; sql: string }> = [
  ...
  {
    name: '008_enable_row_level_security',
    sql: `
  ...
  {
    name: '009_create_users_rls_policies',
    sql: `
  ...
  {
    name: '010_create_gold_purchases_rls_policies',
    sql: `
  ...
  {
    name: '011_create_gold_prices_rls_policies',
    sql: `
  ...
  {
    name: '012_create_payments_rls_policies',
    sql: `
  ...
  {
    name: '013_create_transactions_rls_policies',
    sql: `
  ...
  {
    name: '014_create_market_insights_rls_policies',
    sql: `
  ...
  {
    name: '015_create_new_user_trigger',
    sql: `
  ...
  try {
    ...
    for (const migration of migrations) {
      try {
        ...
        if (error) {
          console.warn(`Migration ${migration.name} failed (may already exist):`, error.message)
        } else {
...
export async function runDirectMigrations() {
  const supabase = await createClient()
  ...
  try {
    ...
    for (const migration of migrations) {
      try {
        ...
        if (sqlError) {
          console.warn(`Migration ${migration.name} warning:`, sqlError.message)
        } else {
          console.log(`✓ Migration ${migration.name} completed`)
          await supabase.from('migrations').insert({ name: migration.name, applied_at: new Date() })
        }
      } catch (err: any) {
        console.warn(`Migration ${migration.name} warning:`, err.message)
      }
